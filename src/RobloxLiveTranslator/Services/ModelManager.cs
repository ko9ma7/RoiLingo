...
{
    private static readonly ConcurrentDictionary<string, SemaphoreSlim> LanguageLocks = new(StringComparer.OrdinalIgnoreCase);
    ...
    public async Task EnsureLanguagesAsync(string languages, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var lang in ParseLanguages(languages))
        {
            ...
            var gate = LanguageLocks.GetOrAdd(lang, _ => new SemaphoreSlim(1, 1));
